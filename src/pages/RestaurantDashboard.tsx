    ...
    <div className="min-h-screen bg-gray-50 pt-16 pb-20">
      <div className="max-w-lg mx-auto px-4 pt-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-black text-gray-900">{t('Dashboard', 'Panel')}</h1>
        ...
        {/* EcoScore card */}
        <div className="bg-gradient-to-br from-green-700 to-emerald-600 rounded-3xl p-5 text-white mb-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              <p className="text-white/70 text-sm">{t('Your EcoScore', 'Tu EcoScore')}</p>
              <div className="flex items-end gap-2">
                <span className="text-5xl font-black">{ecoScore.total}</span>
        ...
        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm mb-5">
          ...
          <div className="space-y-3">
            ...
            <EcoScoreBar label={t('Transparency', 'Transparencia')} value={ecoScore.transparencyScore} max={10} color="bg-purple-500" />
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm mb-5">
          ...
          <div className="bg-gray-50 rounded-xl p-3 mb-4">
            <div className="flex items-baseline gap-1">
              <span className="text-3xl font-black text-gray-900">{ecoScore.weeklyEmissions}</span>
              <span className="text-gray-500">kg CO₂e</span>
            </div>
            ...
            {offsetOptions.map((opt, i) => (
              ...
              >
                ...
                <div className="text-right">
                  <p className="text-xs text-green-600 font-semibold">+{opt.points} pts</p>
                </div>
              </button>
            ))}
          </div>

          <button className="mt-4 w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-xl transition-colors flex items-center justify-center gap-2">
            <DollarSign size={16} />
        ...
        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm mb-5">
          ...
          {/* Progress chart */}
          <div className="bg-gray-50 rounded-xl p-3 mb-4">
            <p className="text-xs text-gray-500 mb-3">{t('Waste % (last 4 weeks)', 'Desperdicio % (últimas 4 semanas)')}</p>
            <div className="flex items-end gap-2 h-16">
              {wasteHistory.map((w) => (
                <div key={w.week} className="flex-1 flex flex-col items-center gap-1">
                  ...
                  <span className="text-xs text-gray-400 whitespace-nowrap">{w.pct}%</span>
                  <span className="text-xs text-gray-300">{w.week.split(' ')[1]}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="font-bold text-green-700">{wastePct}%</span>
            </div>
            <input
              type="range"
              min={0}
          ...
          <div className="mt-4 grid grid-cols-2 gap-2">
            {[
              { icon: '🗑️', label: t('Thrown away', 'Tirado'), en: '', es: '' },
              { icon: '🤝', label: t('Donated', 'Donado'), en: '', es: '' },
              { icon: '🌱', label: t('Composted', 'Compostado'), en: '', es: '' },
              { icon: '🏷️', label: t('Sold at discount', 'Vendido con descuento'), en: '', es: '' },
            ].map((item) => (
              <div key={item.label} className="bg-gray-50 rounded-xl p-2.5">
                ...
                <input
                  type="number"
                  placeholder="kg"
                  className="w-full text-sm font-semibold bg-transparent outline-none text-gray-800 placeholder-gray-300"
                  min={0}
                />
              </div>
            ))}
        ...
        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm mb-5">
          ...
          <div className="flex items-center justify-between bg-gray-50 rounded-2xl p-4 mb-3">
            <div>
              <p className="text-gray-500 text-sm">{t('Projected EcoScore', 'EcoScore Proyectado')}</p>
              <div className="flex items-baseline gap-1">
                <span className="text-4xl font-black text-green-700">{simulatedScore}</span>
        ...
        <div className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm mb-5">
          ...
          <p className="text-sm text-gray-500 mb-4">
            {t('Upload receipts, invoices, and waste logs to improve your Transparency score.', 'Sube recibos, facturas y registros de desperdicio para mejorar tu puntuación de Transparencia.')}
          </p>
          <div className="space-y-2">
            {[
              ...
              <button
                ...
                className="w-full flex items-center gap-3 border-2 border-dashed border-gray-200 hover:border-green-400 hover:bg-green-50 rounded-xl p-3 text-left transition-all group"
              >
                <span className="text-2xl">{item.icon}</span>
                <span className="flex-1 text-sm text-gray-600 group-hover:text-green-700">{item.label}</span>
                <Upload size={16} className="text-gray-300 group-hover:text-green-500" />
              </button>
            ))}
          </div>

          {showUploadSuccess && (
            <div className="mt-3 flex items-center gap-2 bg-green-50 border border-green-200 rounded-xl px-3 py-2 text-green-700">
              <CheckCircle size={16} />
              <p className="text-sm font-semibold">
        ...
        <div className="bg-orange-50 border border-orange-100 rounded-2xl p-4 mb-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <button className="bg-orange-500 hover:bg-orange-600 text-white font-bold text-sm px-4 py-2 rounded-xl transition-colors">
